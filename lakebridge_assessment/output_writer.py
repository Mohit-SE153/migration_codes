"""
Writes the Lakebridge Assessment output contract -- per-category JSON,
an aggregate manifest, CSV rollups, and a Markdown report. Independent
reimplementation of assessment/output_writer.py (not imported -- see
schema.py's module docstring), including the same ANSI/newline
sanitization for Markdown table cells (Lakebridge's own report text can
carry the same kind of messy multi-line notes sqlglot's parse errors do).
"""
...
from lakebridge_assessment.logging_setup import logger
...
def write_manifest_json(manifest: AssessmentManifest, output_dir: str, filename: str = "lakebridge_assessment_manifest.json") -> Path:
...
def write_csv_rollup(manifest: AssessmentManifest, output_dir: str, filename: str = "lakebridge_assessment_rollup.csv") -> Path:
    out_path = Path(output_dir) / filename
    out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    if summary is not None:
        ...
        rows.append({"category": "objects_without_native_complexity", "key": "(all)", "count": summary.objects_without_native_complexity})
...
def write_object_complexity_csv(manifest: AssessmentManifest, output_dir: str, filename: str = "object_complexity.csv") -> Path:
    out_path = Path(output_dir) / filename
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = ["object_type", "name", "complexity_tier", "estimated_hours", "fan_in", "fan_out", "compatibility_flags"]
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for oc in sorted(manifest.object_complexity, key=lambda o: o.complexity_score, reverse=True):
            writer.writerow({
                ...
                "estimated_hours": oc.estimated_hours, "fan_in": oc.fan_in, "fan_out": oc.fan_out,
                "compatibility_flags": "; ".join(oc.compatibility_flags),
            })
    return out_path
...
def write_markdown_report(manifest: AssessmentManifest, output_dir: str, filename: str = "lakebridge_assessment_report.md") -> Path:
    out_path = Path(output_dir) / filename
    out_path.parent.mkdir(parents=True, exist_ok=True)
    summary = manifest.summary

    lines: list[str] = [
        f"# Lakebridge Migration Assessment Report -- {manifest.database}",
        "",
        f"Generated: {manifest.generated_at}",
        f"Source Lakebridge Discovery manifest: `{manifest.source_manifest_path}`",
        f"Complexity source: **{manifest.complexity_source}** (Lakebridge Analyzer's own per-object rating, not independently computed)",
        "",
        f"> {manifest.mapping_notes}" if manifest.mapping_notes else "",
        "",
        "## Executive summary",
        "",
    ]
    ...
    if summary is not None:
        lines += [
            f"- **{summary.total_objects_scored}** objects carried a native Lakebridge complexity rating and were scored "
            f"({summary.objects_without_native_complexity} other object(s) had no complexity value in the report and were skipped).",
            ...
            "",
            "### Complexity tier breakdown (Lakebridge's own ratings)",
            "",
            _md_table(["Tier", "Object count"], sorted(summary.complexity_tier_counts.items())),
            "",
            "### Top riskiest objects (by Lakebridge tier, ties broken by dependency fan-in/out)",
            "",
