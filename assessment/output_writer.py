"""
Writes the Assessment-phase output contract: one JSON file per category
(mirroring autovista/output_writer.py's per-category convention), an
aggregate assessment_manifest.json, CSV rollups for quick review in a
spreadsheet, and a human-readable Markdown report.
"""
...
from assessment.logging_setup import logger
...
def write_manifest_json(manifest: AssessmentManifest, output_dir: str, filename: str = "assessment_manifest.json") -> Path:
...
def write_csv_rollup(manifest: AssessmentManifest, output_dir: str, filename: str = "assessment_rollup.csv") -> Path:
    out_path = Path(output_dir) / filename
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def _md_cell(value) -> str:
    """Markdown table cells are one line each -- raw newlines split a row
    across multiple lines and break the table, and sqlglot parse-error
    text can carry both newlines and ANSI highlight codes (see the parse
    error strings in unsupported_objects.json). Strips those, collapses
    whitespace, escapes literal `|`, and truncates (full text is still
    available untouched in the JSON/CSV outputs)."""
...
def write_markdown_report(manifest: AssessmentManifest, output_dir: str, filename: str = "assessment_report.md") -> Path:
    out_path = Path(output_dir) / filename
    out_path.parent.mkdir(parents=True, exist_ok=True)
    summary = manifest.summary

    lines: list[str] = [
        f"# Migration Assessment Report -- {manifest.database}",
        "",
    ...
    if summary is not None:
        lines += [
            f"- **{summary.total_objects_scored}** objects scored (tables, views, stored procedures, functions, triggers, SSIS embedded SQL).",
